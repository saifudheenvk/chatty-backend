...
resource "aws_security_group" "alb_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "TCP"
    cidr_blocks = [var.global_destination_cidr_block] # access internet from anywhere
...
resource "aws_security_group" "autoscaling_group_sg" {
  ...
  ingress {
    ...
    protocol        = "TCP"
    security_groups = [aws_security_group.alb_sg.id]
    description     = "Allows HTTP traffic into web server thorough ALB"
  }
  ...
  ingress {
    ...
    description     = "Allows Access to Web Server from Bastion Host"
  }
