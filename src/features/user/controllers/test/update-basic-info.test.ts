...
describe('EditBasicInfo', () => {
  ...
  describe('info', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      for (const [key, value] of Object.entries(req.body)) {
        expect(UserCache.prototype.updateSingleUserFromcache).toHaveBeenCalledWith(`${req.currentUser?.userId}`, key, `${value}`);
      }
  ...
  describe('social', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      expect(UserCache.prototype.updateSingleUserFromcache).toHaveBeenCalledWith(`${req.currentUser?.userId}`, 'social', req.body);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'Updated successfully'
      });
