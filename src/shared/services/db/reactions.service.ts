...
class ReactionService {
  public async addReactionDataToDB(data: IReactionJob): Promise<void> {
    const { postId, userTo, userFrom, username, type, previousReaction, reactionObject } = data;
    ...
    const updatedReaction: [ IUserDocument, IReactionDocument, IPostDocument] = (await Promise.all([
      ...
      ReactionModel.replaceOne({postId: postId, type: previousReaction, username}, updatedReactionObject, { upsert: true }),
    ...
    if(updatedReaction[0] && updatedReaction[0].notifications.reactions && userFrom !== userTo) {
      ...
      socketIONotificationObject.emit('insert notification', notifications, { userTo: userTo as string });
  ...
  public async getSinglePostReactionByUsername(postId: string, username: string): Promise<[IReactionDocument, number] | []> {
    const reactions: IReactionDocument[] = await ReactionModel.aggregate([{$match: { postId: new mongoose.Types.ObjectId(postId), username: Helper.convertFirstLetterUppercase(username)}}]);
    return [reactions[0], 1];
  }

  public async getReactionsByUsername(username: string): Promise<IReactionDocument[]> {
    const reactions: IReactionDocument[] = await ReactionModel.aggregate([{$match: {username: Helper.convertFirstLetterUppercase(username)}}]);
    return reactions;
  }
}
